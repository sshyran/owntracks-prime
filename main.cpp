...
#warning TLS!!!
                mqttSocket = MQTTSocket();
                int rc = mqttSocket.connect(mqtt_server, mqtt_port);
                if (rc != 0) {
        ...
        if (socketConnect) {
            if (!mqttConnect) {
                ...
                snprintf(clientID, sizeof(clientID), "prime-gsm-%s", deviceID);
                ...
                if ( (rc = client->connect(data)) != 0) {
                    ...
                    mqttConnect = true;
                    snprintf(pubtopic, sizeof(pubtopic), "%s/%s/start", BASETOPIC, deviceID);
                    snprintf(payload, sizeof(payload), "%s x.y.z %d", deviceID, time(NULL));

                    if ( (rc = client->publish(pubtopic, payload, strlen(payload), MQTT::QOS1, true)) != 0 ) {
